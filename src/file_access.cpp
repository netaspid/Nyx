...
bool user_id_from_hex(const std::string& hex, UserId& out) {
  ByteBuffer buf;
  if (!from_hex(hex, buf) || buf.size() != out.size())
    return false;
...
FileRole parse_role(const std::string& obj) {
  FileRole role;
  if (auto id = json_get_string(obj, "id"))
    role.id = *id;
  if (auto name = json_get_string(obj, "name"))
    role.name = *name;
  if (auto perms = json_get_uint(obj, "permissions"))
    role.permissions = *perms;
  if (auto builtin = json_get_bool(obj, "builtin"))
...
FileMemberAssignment parse_assignment(const std::string& obj) {
  ...
  if (auto uid = json_get_string(obj, "user_id"))
    user_id_from_hex(*uid, a.user_id);
  if (auto rid = json_get_string(obj, "role_id"))
    a.role_id = *rid;
  return a;
}
...
FileRootGrant parse_root_grant(const std::string& obj) {
  ...
  if (auto rel = json_get_string(obj, "rel"))
    g.relative_path = *rel;
  if (auto uid = json_get_string(obj, "user_id"))
    user_id_from_hex(*uid, g.user_id);
  if (auto rid = json_get_string(obj, "role_id"))
    g.role_id = *rid;
  if (auto direct = json_get_uint(obj, "direct"))
    g.direct_permissions = *direct;
  if (auto direct_only = json_get_bool(obj, "direct_only"))
    g.direct_only = *direct_only;
  return g;
}
...
std::string grant_effective_share_root(const FileRootGrant& g) {
  ...
  const auto combined = path_from_utf8(g.root_path) / path_from_utf8(g.relative_path);
...
uint32_t grant_role_permissions(const GroupFileAccess& policy, const FileRootGrant& g) {
  if (g.direct_only)
    return g.direct_permissions;
  if (!g.role_id.empty())
    return role_permissions(policy, g.role_id);
  return static_cast<uint32_t>(FilePermission::List);
}
...
bool permissions_from_share_root_grant(const GroupFileAccess& policy,
  ...
  for (const auto& g : policy.root_grants) {
    if (g.relative_path.empty())
      continue;
    if (grant_effective_share_root(g) != root_norm)
      continue;
  ...
  for (const auto& g : policy.root_grants) {
    ...
    if (grant_effective_share_root(g) != root_norm)
      continue;
    if (g.user_id != FileAccessStore::path_role_user())
      continue;
    out = grant_role_permissions(policy, g);
    return true;
  }
...
const FileRootGrant* find_path_grant(const GroupFileAccess& policy,
  ...
  for (const auto& g : policy.root_grants) {
    if (normalize_grant_root(g.root_path) != root_norm)
      continue;
    if (path_to_posix_copy(g.relative_path) != rel_posix)
      continue;
    if (g.user_id != user_id)
      continue;
    return &g;
...
void write_group_policy_json(std::ostream& out, const GroupFileAccess& p) {
  ...
  for (std::size_t pi = 0; pi < p.permission_presets.size(); ++pi) {
    ...
    if (pi)
      out << ',';
    out << "{\"id\":\"" << json_escape(preset.id) << "\",\"name\":\"" << json_escape(preset.name)
  ...
  for (std::size_t gi2 = 0; gi2 < p.root_grants.size(); ++gi2) {
    ...
    out << "{\"root\":\"" << json_escape(g.root_path) << "\",\"rel\":\""
        ...
        << "\",\"role_id\":\"" << json_escape(g.role_id) << "\",\"direct\":" << g.direct_permissions
        << ",\"direct_only\":" << (g.direct_only ? "true" : "false") << "}";
  }
  out << "]}";
}
...
std::string FileAccessStore::store_path() {
  return data_dir() + "/file_access.json";
}

std::string FileAccessStore::role_id_owner() {
  return "owner";
}
std::string FileAccessStore::role_id_member() {
  return "member";
}
std::string FileAccessStore::role_id_viewer() {
  return "viewer";
}

UserId FileAccessStore::path_role_user() {
  return UserId {};
}

GroupFileAccess FileAccessStore::default_policy(const GroupId& group_id, const GroupRecord& group) {
    ...
    for (; pos < json.size(); ++pos) {
      ...
      if (c == '{') {
        ...
        if (depth == 0) {
          ...
          if (!std::all_of(policy.group_id.begin(), policy.group_id.end(), [](uint8_t b) {
                return b == 0;
              })) {
...
GroupFileAccess& FileAccessStore::ensure_policy(const GroupId& group_id, const GroupRecord& group) {
  for (auto& policy : policies_) {
    ...
    for (const auto& m : group.members) {
      const bool known =
          std::any_of(policy.assignments.begin(),
                      policy.assignments.end(),
                      [&](const FileMemberAssignment& a) { return a.user_id == m.user_id; });
      if (!known) {
        FileMemberAssignment a;
        a.user_id = m.user_id;
...
uint32_t FileAccessStore::permissions_for(const GroupId& group_id, const UserId& user_id) const {
...
uint32_t FileAccessStore::permissions_for(const GroupId& group_id,
  ...
  const std::string root_norm =
      root_path.empty() ? std::string {} : normalize_grant_root(root_path);
  ...
  if (!root_path.empty()) {
    if (const FileRootGrant* exact = find_path_grant(*policy, root_norm, rel_posix, user_id)) {
      if (exact->direct_only)
        return exact->direct_permissions;
      if (!exact->role_id.empty())
        return role_permissions(*policy, exact->role_id);
    }
    ...
    while (true) {
      ...
      if (const FileRootGrant* ancestor = find_path_grant(*policy, root_norm, walk, user_id)) {
        if (ancestor->direct_only)
          return ancestor->direct_permissions;
        if (!ancestor->role_id.empty())
          return role_permissions(*policy, ancestor->role_id);
      }
      if (walk.empty())
        break;
    }
...
bool FileAccessStore::set_path_role(const GroupId& group_id,
                                    const std::string& root_path,
...
bool FileAccessStore::set_path_member_role(const GroupId& group_id,
  ...
  if (role_id.empty()) {
    policy.root_grants.erase(
        std::remove_if(policy.root_grants.begin(),
  ...
  for (const auto& r : policy.roles) {
    ...
    policy.root_grants.push_back({root_norm, rel_posix, user_id, role_id, 0, false});
    return save();
  }
  return false;
}
...
bool FileAccessStore::upsert_permission_preset(const GroupId& group_id,
  ...
  if (preset.id.empty() || preset.name.empty())
    return false;
  auto& policy = policy_for(group_id);
...
bool FileAccessStore::remove_permission_preset(const GroupId& group_id,
  ...
  auto& policy = policy_for(group_id);
  const auto it = std::remove_if(policy.permission_presets.begin(),
                                 policy.permission_presets.end(),
                                 [&](const FilePermissionPreset& p) { return p.id == preset_id; });
  if (it == policy.permission_presets.end())
    return false;
  policy.permission_presets.erase(it, policy.permission_presets.end());
  return save();
}

bool FileAccessStore::set_member_role(const GroupId& group_id,
                                      const UserId& user_id,
                                      const std::string& role_id) {
  auto& policy = policy_for(group_id);
...
bool FileAccessStore::upsert_role(const GroupId& group_id, const FileRole& role) {
  ...
  for (auto& r : policy.roles) {
    if (r.id == role.id) {
      if (r.builtin && role.id == role_id_owner())
        return false;
      r.name = role.name;
...
bool FileAccessStore::remove_role(const GroupId& group_id, const std::string& role_id) {
  ...
  const auto it = std::find_if(
      policy.roles.begin(), policy.roles.end(), [&](const FileRole& r) { return r.id == role_id; });
  if (it == policy.roles.end() || it->builtin)
    return false;
  for (auto& a : policy.assignments) {
    if (a.role_id == role_id)
      a.role_id = role_id_viewer();
  }
  for (auto& g : policy.root_grants) {
    if (g.role_id == role_id)
      g.role_id = role_id_viewer();
  }
...
bool FileAccessStore::import_policy(const GroupFileAccess& incoming) {
  if (std::all_of(
          incoming.group_id.begin(), incoming.group_id.end(), [](uint8_t b) { return b == 0; })) {
    return false;
  }
  for (auto& p : policies_) {
    if (p.group_id != incoming.group_id)
      continue;
    p = incoming;
    return save();
  }
