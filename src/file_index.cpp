...
std::string FileEntry::absolute_path() const {
  return path_to_utf8(
      (path_from_utf8(root_path) / path_from_utf8(relative_path)).lexically_normal());
}
...
std::vector<FileEntry> FileIndex::listing_level(const std::vector<FileEntry>& source,
  ...
  auto take_share_group = [&](const FileEntry& e) {
    if (share_group_set)
      return;
    if (!std::all_of(
            e.share_group.begin(), e.share_group.end(), [](uint8_t b) { return b == 0; })) {
  ...
  for (const auto& e : source) {
    if (normalize_utf8_path(e.root_path) != root_norm)
      continue;
    take_share_group(e);
...
std::vector<FileEntry> FileIndex::listing_at_root(const std::string& share_root_path,
  ...
  for (const auto& e : entries_) {
    if (e.is_directory())
      continue;
    if (normalize_utf8_path(e.root_path) == norm)
      in_root.push_back(e);
  }
...
std::string FileIndex::index_path() {
  return data_dir() + "/file_index.json";
}
...
bool FileIndex::add_root(const std::string& root_path,
                         const GroupId* group_id,
                         ScanProgressFn progress) {
  std::error_code ec;
  const std::string norm = normalize_utf8_path(root_path);
  if (!std::filesystem::exists(path_from_utf8(norm), ec))
    return false;
...
bool FileIndex::remove_root(const std::string& root_path, const GroupId* group_id) {
  ...
  std::lock_guard lock(mutex_);
  const auto root_it =
      std::remove_if(share_roots_.begin(), share_roots_.end(), [&](const ShareRoot& r) {
        return normalize_utf8_path(r.path) == norm && r.group_id == gid;
      });
  if (root_it == share_roots_.end())
    return false;
...
std::vector<ShareRoot> FileIndex::roots_for_session(const GroupId& session_group) const {
  ...
  for (const auto& r : share_roots_) {
    ...
    if (entry_visible_in_session(fake, session_group))
      out.push_back(r);
  }
  return out;
}
...
std::vector<FileEntry> FileIndex::entries_for_session(const GroupId& session_group) const {
  ...
  for (const auto& e : entries_) {
    if (entry_visible_in_session(e, session_group))
      out.push_back(e);
  }
  return out;
}
...
int FileIndex::count_in_root(const std::string& root_path, const GroupId& scope_group) const {
  const std::string norm = normalize_utf8_path(root_path);
  std::lock_guard lock(mutex_);
  int count = 0;
  for (const auto& e : entries_) {
    if (e.is_directory())
      continue;
    if (normalize_utf8_path(e.root_path) == norm && e.share_group == scope_group)
      ++count;
  }
  return count;
}

FileEntry FileIndex::make_directory_marker(const ShareRoot& root,
                                           int file_count,
  ...
  const std::string key = "nyx-dir:" + group_id_hex(root.group_id) + ":" + root.path;
  marker.hash = hash_bytes(reinterpret_cast<const uint8_t*>(key.data()), key.size());
  return marker;
}
...
std::vector<FileEntry> FileIndex::listing_for_session(const GroupId& session_group) const {
  ...
  for (const auto& r : share_roots_) {
    ...
    for (const auto& e : entries_) {
      if (e.is_directory())
        continue;
      if (normalize_utf8_path(e.root_path) == norm && e.share_group == r.group_id)
        ++count;
    }
...
bool FileIndex::rescan_root(const std::string& root_path,
                            const GroupId* group_id,
                            ScanProgressFn progress) {
  const std::string norm = normalize_utf8_path(root_path);
  GroupId gid {};
...
std::optional<FileEntry> FileIndex::find_for_session(const FileHash& hash,
  ...
  for (const auto& entry : entries_) {
    if (entry.hash != hash || entry.is_directory())
      continue;
    if (entry_visible_in_session(entry, session_group))
      return entry;
  }
  return std::nullopt;
}
    ...
    if (arr_start != std::string::npos && arr_end != std::string::npos && arr_end > arr_start) {
      for (const auto& obj : json_split_objects(json.substr(arr_start, arr_end - arr_start + 1))) {
        ...
        if (end == std::string::npos)
          continue;
        if (!hash_from_hex(obj.substr(i, end - i), entry.hash))
          continue;
        ...
        if (auto rel = json_get_string(obj, "rel"))
          entry.relative_path = *rel;
        if (auto mime = json_get_string(obj, "mime"))
          entry.mime = *mime;
        ...
        const bool had_group = json_get_string(obj, "group").has_value();
        if (auto gid = json_get_string(obj, "group")) {
        ...
        const bool has_root =
            std::any_of(share_roots_.begin(), share_roots_.end(), [&](const ShareRoot& root) {
              return normalize_utf8_path(root.path) == entry.root_path &&
                     root.group_id == entry.share_group;
            });
        const std::string managed_prefix = normalize_utf8_path(data_dir() + "/objects") + "/";
        const bool is_managed = entry.root_path.rfind(managed_prefix, 0) == 0;
        std::error_code ec;
        if ((has_root || is_managed) &&
            std::filesystem::is_regular_file(path_from_utf8(entry.absolute_path()), ec)) {
...
bool FileIndex::save() const {
  ...
  for (std::size_t i = 0; i < share_roots_.size(); ++i) {
    ...
    file << "{\"root\":\"" << json_escape(r.path) << "\",\"group\":\"" << group_id_hex(r.group_id)
         << "\"}";
  }
  file << "],\"files\":[";
...
std::string FileIndex::library_owner_dir(const GroupId& scope_group, const UserId& owner_id) {
  const std::string root = library_root_path(scope_group);
  if (user_id_is_zero(owner_id))
    return root;
...
bool FileIndex::ensure_library_root(const GroupId& scope_group) {
  ...
  for (const auto& existing : share_roots_) {
    if (normalize_utf8_path(existing.path) == path && existing.group_id == scope_group) {
      return true;
    }
  }
  ShareRoot sr;
...
std::optional<FileEntry> FileIndex::adopt_file(const std::string& source_path,
  ...
  if (!ensure_library_root(scope_group))
    return std::nullopt;

  std::string safe_name = path_to_utf8(path_from_utf8(display_name).filename());
  if (safe_name.empty())
    safe_name = hash_hex(expected_hash);
  const std::string library_root = library_root_path(scope_group);
  UserId owner {};
  if (owner_id && !user_id_is_zero(*owner_id))
    owner = *owner_id;
  ...
  if (source_fs.lexically_normal() != object_fs.lexically_normal()) {
    std::filesystem::copy_file(
        source_fs, object_fs, std::filesystem::copy_options::overwrite_existing, ec);
    if (ec)
      return std::nullopt;
  }
  ...
  std::string library_path = path_to_utf8(path_from_utf8(library_dir) / path_from_utf8(leaf_name));
  {
    ...
    while (std::filesystem::exists(path_from_utf8(library_path), ec)) {
      ...
      if (hash_file(library_path, existing_hash) && existing_hash == expected_hash) {
        break;
      }
      ...
      if (dot == std::string::npos) {
        ...
        leaf_name =
            safe_name.substr(0, dot) + "-" + std::to_string(suffix++) + safe_name.substr(dot);
      }
      library_path = path_to_utf8(path_from_utf8(library_dir) / path_from_utf8(leaf_name));
    }
  }
  const auto library_fs = path_from_utf8(library_path);
  if (object_fs.lexically_normal() != library_fs.lexically_normal()) {
    std::filesystem::copy_file(
        object_fs, library_fs, std::filesystem::copy_options::overwrite_existing, ec);
    if (ec)
      return std::nullopt;
  }
  ...
  entry.size = static_cast<uint64_t>(std::filesystem::file_size(library_fs, ec));
  if (ec)
    return std::nullopt;
...
std::optional<FileEntry> FileIndex::import_file(const std::string& source_path,
                                                const std::string& display_name,
                                                const std::string& mime,
                                                const GroupId& scope_group,
                                                const UserId* owner_id,
