  ...
  for (int i = 0; i < 128; ++i) {
    if (bits[static_cast<std::size_t>(i)]) {
      entropy[static_cast<std::size_t>(i / 8)] =
          static_cast<uint8_t>(entropy[static_cast<std::size_t>(i / 8)] | (1u << (7 - (i % 8))));
    }
  }
  return entropy;
}
...
std::vector<int> bits_to_indices(const std::vector<bool>& bits) {
  ...
  for (std::size_t i = 0; i + 11 <= bits.size(); i += 11) {
    int v = 0;
    for (int b = 0; b < 11; ++b)
      v = (v << 1) | (bits[i + static_cast<std::size_t>(b)] ? 1 : 0);
    out.push_back(v);
  }
  return out;
}
...
bool phrase_checksum_ok(const std::vector<int>& indices) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    if (bits[128 + static_cast<std::size_t>(i)] != expected)
      return false;
  }
  return true;
}
...
std::string generate_recovery_phrase() {
  ...
  for (int i = 0; i < 4; ++i)
    bits.push_back(((hash[0] >> (7 - i)) & 1) != 0);

  const auto indices = bits_to_indices(bits);
  std::vector<std::string> words;
  words.reserve(12);
  for (int idx : indices)
    words.emplace_back(kBip39Words[idx]);
...
bool normalize_recovery_phrase(const std::string& phrase,
  ...
  if (words.size() != 12) {
    if (err)
      *err = "нужно ровно 12 слов recovery-фразы";
    return false;
  }
  ...
  if (!phrase_checksum_ok(indices)) {
    if (err)
      *err = "фраза повреждена (неверный checksum)";
    return false;
  }
  if (normalized_out)
    *normalized_out = join_words(words);
  return true;
}
