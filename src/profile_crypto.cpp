...
void pbkdf2_sha256(const std::string& password,
                   const uint8_t* salt,
                   std::size_t salt_len,
  ...
  for (std::uint32_t i = 1; i < iterations; ++i) {
    hmac_sha256(
        reinterpret_cast<const uint8_t*>(password.data()), password.size(), u.data(), 32, u.data());
    for (int j = 0; j < 32; ++j)
      t[j] ^= u[j];
  }
...
bool mac_verify(const uint8_t key[32],
                const uint8_t* nonce,
                const uint8_t* cipher,
                std::size_t cipher_len,
                const uint8_t mac[kMacSize]) {
...
void mac_compute(const uint8_t key[32],
                 const uint8_t* nonce,
                 const uint8_t* cipher,
                 std::size_t cipher_len,
...
bool encrypt_profile_blob(const Profile& profile,
...
bool save_encrypted_profile(const std::string& path,
  ...
  std::vector<uint8_t> blob;
  if (!encrypt_profile_blob(profile, key, salt.data(), blob))
    return false;

  std::ofstream out(path_from_utf8(path), std::ios::binary | std::ios::trunc);
  if (!out) {
    if (err)
      *err = "не удалось записать профиль";
    return false;
  }
...
bool load_encrypted_profile(const std::string& path,
  ...
  if (blob.size() < header) {
    if (err)
      *err = "повреждённый профиль";
    return false;
  }
  if (std::memcmp(blob.data(), kProfileMagic, 4) != 0) {
    if (err)
      *err = "неизвестный формат профиля";
    return false;
  }
  ...
  if (version != kFormatVersion) {
    if (err)
      *err = "версия профиля не поддерживается";
    return false;
  }
  if (blob[6] != 1) {
    if (err)
  ...
  if (!parse_profile_json(json, out)) {
    if (err)
      *err = "не удалось разобрать профиль";
    return false;
  }
  return true;
}
...
bool save_encrypted_profile_with_key(const std::string& path,
  ...
  std::vector<uint8_t> blob;
  if (!encrypt_profile_blob(profile, derived_key, salt.data(), blob))
    return false;
  std::ofstream out(path_from_utf8(path), std::ios::binary | std::ios::trunc);
  if (!out) {
    if (err)
...
bool load_encrypted_profile_with_key(const std::string& path,
  ...
  if (!mac_verify(derived_key.data(), nonce, cipher, cipher_len, mac)) {
    if (err)
      *err = "сохранённая сессия не подходит к профилю";
    return false;
  }
  std::vector<uint8_t> plain(cipher_len);
