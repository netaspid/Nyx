...
Frame Frame::make(PacketType type, uint32_t stream_id, uint32_t seq, ByteBuffer payload) {
  Frame f;
...
std::optional<Frame> Frame::decode(const uint8_t* data, std::size_t len, std::string* err) {
  if (len < kHeaderSize) {
    if (err)
...
std::optional<EndpointHint> EndpointHint::decode(const uint8_t* data, std::size_t len) {
  if (len < 26)
    return std::nullopt;
  EndpointHint h;
...
std::optional<RendezvousMessage> RendezvousMessage::decode(const uint8_t* data, std::size_t len) {
  if (len < 1)
    return std::nullopt;
  RendezvousMessage m;
  switch (data[0]) {
...
std::optional<ControlMessage> ControlMessage::decode(const uint8_t* data, std::size_t len) {
  if (len < 1)
    return std::nullopt;
  ControlMessage m;
