...
RendezvousClient::RendezvousClient(UdpSocket socket, std::string server_host, uint16_t server_port)
    : socket_(std::move(socket)), server_host_(std::move(server_host)), server_port_(server_port) {}
...
std::optional<EndpointHint> RendezvousClient::lookup(const InviteToken& token) {
  ...
  msg.token = token;
  if (!send_msg(PacketType::RendezvousLookup, msg.encode()))
    return std::nullopt;
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    if (!data)
      continue;
    if (!endpoint_matches(host, port, server_host_, server_port_))
      continue;
