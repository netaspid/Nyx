...
bool parse_key_hex(const std::string& hex, std::array<uint8_t, 32>& out) {
  ByteBuffer bytes;
  if (!from_hex(hex, bytes) || bytes.size() != 32)
    return false;
...
bool save_profile(const std::string& path, const Profile& profile) {
  ...
  if (!file)
    return false;
  file << "{\"v\":1,\"nickname\":\"" << json_escape(profile.nickname) << "\",\"sk\":\""
...
bool load_profile(const std::string& path, Profile& out) {
  ...
  if (!parse_key_hex(*sk, profile.secret_key))
    return false;
  if (!parse_key_hex(*pk, profile.public_key))
    return false;
...
bool ContactBook::load() {
  ...
  while ((pos = json.find("\"id\":\"", pos)) != std::string::npos) {
    ...
    Contact contact;
    if (!parse_key_hex(id_hex, contact.user_id))
      continue;
    ...
    if (seen_key != std::string::npos && seen_key < nick_end + 64) {
      try {
        contact.last_seen_ms = std::stoull(json.substr(seen_key + 12));
    ...
    if (bio_key != std::string::npos && bio_key < nick_end + 400) {
      ...
      const auto end = json.find('"', start);
      if (end != std::string::npos)
        contact.bio = json.substr(start, end - start);
    }
    ...
    if (int_key != std::string::npos && int_key < nick_end + 500) {
      ...
      const auto end = json.find('"', start);
      if (end != std::string::npos)
        contact.interests = json.substr(start, end - start);
    }
...
bool ContactBook::save() const {
  ...
  for (std::size_t i = 0; i < contacts_.size(); ++i) {
    ...
    file << "{\"id\":\"" << to_hex(c.user_id.data(), c.user_id.size()) << "\",\"nickname\":\""
         << json_escape(c.nickname) << "\",\"trust\":" << static_cast<int>(c.trust_level)
...
void ContactBook::upsert(Contact contact) {
  for (auto& existing : contacts_) {
    if (existing.user_id == contact.user_id) {
      ...
      if (!contact.photo_hashes.empty())
        existing.photo_hashes = contact.photo_hashes;
      return;
    }
  }
  contacts_.push_back(std::move(contact));
...
bool ContactBook::remove(const UserId& user_id) {
  const auto it = std::remove_if(
      contacts_.begin(), contacts_.end(), [&](const Contact& c) { return c.user_id == user_id; });
  if (it == contacts_.end())
    return false;
