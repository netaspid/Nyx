...
OpusEncoder* as_enc(void* p) {
  return static_cast<OpusEncoder*>(p);
}
OpusDecoder* as_dec(void* p) {
...
OpusDecoderWrap::~OpusDecoderWrap() {
  if (dec_)
    opus_decoder_destroy(as_dec(dec_));
}
...
std::optional<std::vector<int16_t>> OpusDecoderWrap::decode(const uint8_t* data, std::size_t len) {
  ...
  const int n = opus_decode(
      as_dec(dec_), data, static_cast<opus_int32>(len), pcm.data(), kCallAudioFrameSamples, 0);
  if (n < 0)
    return std::nullopt;
