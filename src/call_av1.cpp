...
std::vector<ByteBuffer> fragment_av1_frame(uint16_t frame_id, bool keyframe,
  ...
  if (encoded.empty() || max_payload <= CallVideoFragHeader::kSize + 4) return out;
...
std::optional<CallVideoReassembler::Assembled> CallVideoReassembler::push(
  ...
  if (missing >= 0) {
    if (parity_.empty() || total_size_ == 0) return std::nullopt;
    ...
    if (off >= total_size_) return std::nullopt;
...
std::optional<Av1Decoder::Frame> Av1Decoder::decode(const uint8_t* data, std::size_t len) {
  ...
  if (img->x_chroma_shift != 1 || img->y_chroma_shift != 1) return std::nullopt;
  ...
  auto copy_plane = [high_bit_depth, shift](uint8_t* dst, int dst_stride,
  ...
  if (!copy_plane(f.i420.data(), f.width, img->planes[0], img->stride[0],
                  f.width, f.height))
    return std::nullopt;
  ...
  if (!copy_plane(u, f.width / 2, img->planes[u_index], img->stride[u_index],
                  f.width / 2, f.height / 2) ||
      !copy_plane(v, f.width / 2, img->planes[v_index], img->stride[v_index],
                  f.width / 2, f.height / 2))
    return std::nullopt;
  return f;
}
