...
bool resolve_host(const std::string& host,
                  uint16_t port,
...
void close_socket(uintptr_t sock) {
  if (sock == static_cast<uintptr_t>(kInvalidSocket))
    return;
#ifdef _WIN32
...
in_addr resolve_mcast_iface(const std::string& iface_ipv4) {
  ...
  if (ip.empty())
    ip = lan_ipv4_override();
  if (ip.empty()) {
    const std::string guessed = guess_lan_ipv4();
    if (guessed != "127.0.0.1" && guessed != "0.0.0.0")
      ip = guessed;
  }
...
bool join_mcast_group(socket_t s, const std::string& group, in_addr iface, std::string* err) {
  ...
  if (setsockopt(
          s, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq)) !=
      0) {
    if (err)
...
bool leave_mcast_group(socket_t s, const std::string& group, uint32_t iface_nbo) {
  ...
  setsockopt(s, IPPROTO_IP, IP_DROP_MEMBERSHIP, reinterpret_cast<const char*>(&mreq), sizeof(mreq));
  return true;
}
...
bool UdpSocket::send_to(const ByteBuffer& data, const std::string& host, uint16_t port) {
  ...
  socklen_t len = 0;
  if (!resolve_host(host, port, addr, len))
    return false;
...
std::optional<ByteBuffer> UdpSocket::recv_from(std::string& host, uint16_t& port, int timeout_ms) {
  if (!state_)
    return std::nullopt;
  auto s = static_cast<socket_t>(state_->sock);
  if (timeout_ms >= 0) {
...
bool UdpSocket::bind_multicast_listener(const std::string& group,
                                        uint16_t port,
                                        std::string* err,
  ...
  int reuse = 1;
  if (setsockopt(
          s, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuse), sizeof(reuse)) != 0) {
    if (err)
      *err = "SO_REUSEADDR failed";
    return false;
  }
  ...
  if (joined_iface) {
    setsockopt(
        s, IPPROTO_IP, IP_MULTICAST_IF, reinterpret_cast<const char*>(&iface), sizeof(iface));
  }
...
bool UdpSocket::set_multicast_interface(const std::string& ipv4, std::string* err) {
  ...
  if (inet_pton(AF_INET, ipv4.c_str(), &iface) != 1) {
    if (err)
      *err = "bad multicast interface IP";
    return false;
  }
  ...
  if (!state_->mcast_group.empty() && state_->mcast_iface_addr != iface.s_addr) {
    ...
    if (!join_mcast_group(s, state_->mcast_group, iface, &join_err)) {
      in_addr any {};
      any.s_addr = INADDR_ANY;
      if (!join_mcast_group(s, state_->mcast_group, any, err))
        return false;
