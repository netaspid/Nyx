...
bool RendezvousRegistry::allow_request(const std::string& client_ip) {
  ...
  if (bucket.first >= config_.rate_limit_per_minute)
    return false;
  ++bucket.first;
  return true;
}

std::optional<ByteBuffer> RendezvousRegistry::handle_datagram(const std::string& client_ip,
  ...
  if (frame->header.packet_type == PacketType::RendezvousRegister) {
    ...
    if (msg->kind != RendezvousKind::Register)
      return std::nullopt;
  ...
  if (frame->header.packet_type == PacketType::RendezvousLookup) {
    auto msg = RendezvousMessage::decode(frame->payload.data(), frame->payload.size());
    if (!msg || msg->kind != RendezvousKind::Lookup)
      return std::nullopt;
