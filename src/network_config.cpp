...
bool parse_one_host_port(const std::string& part, RendezvousServer& out) {
  std::string host;
  uint16_t port = 0;
  if (!parse_host_port(part, host, port))
    return false;
  out.host = host;
  out.port = port;
  return true;
...
RendezvousServer NetworkConfig::primary_rendezvous() const {
  if (!rendezvous_servers.empty())
    return rendezvous_servers.front();
  ...
  for (char c : trimmed) {
    if (c == ',' || c == ';') {
      ...
      if (!part.empty()) {
        ...
        if (!parse_one_host_port(part, srv))
          return false;
...
bool NetworkConfig::load() {
  ...
  if (auto stun = json_get_string(json, "stun_host"))
    stun_host = *stun;
  ...
  while ((pos = json.find("\"host\":\"", pos)) != std::string::npos) {
    ...
    RendezvousServer srv;
    if (auto h = json_get_string(obj, "host"))
      srv.host = *h;
    std::string port_str = "3478";
    const auto port_key = obj.find("\"port\":");
    if (port_key != std::string::npos) {
      try {
        srv.port = static_cast<uint16_t>(std::stoi(obj.substr(port_key + 7)));
      } catch (const std::exception&) {
        srv.port = 3478;
      }
    }
    if (auto label = json_get_string(obj, "label"))
      srv.label = *label;
    if (!srv.host.empty())
      rendezvous_servers.push_back(std::move(srv));
    pos = obj_end;
  }
...
bool NetworkConfig::save() const {
  ...
  for (std::size_t i = 0; i < rendezvous_servers.size(); ++i) {
    ...
    file << "{\"host\":\"" << json_escape(r.host) << "\",\"port\":" << r.port << ",\"label\":\""
         << json_escape(r.label) << "\"}";
  }
  file << "]}\n";
