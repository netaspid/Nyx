...
std::string auth_prefs_path() {
  return data_root() + "/auth_prefs.json";
}
...
bool protect_bytes(const uint8_t* data, std::size_t len, std::vector<uint8_t>& out) {
  ...
  for (std::size_t i = 0; i < len; ++i)
    out[i] = static_cast<uint8_t>(data[i] ^ mask[i % mask.size()]);
  return true;
#endif
}
...
bool write_remember_token(const std::string& account_id, const std::array<uint8_t, 32>& key) {
  ...
  if (!protect_bytes(key.data(), key.size(), protected_key))
    return false;
  ...
  std::ofstream out(path_from_utf8(remember_path_for(account_id)),
...
bool read_remember_token(const std::string& account_id,
  ...
  if (blob.size() < 4 + 8 + 4)
    return false;
  if (std::memcmp(blob.data(), kRememberMagic, 4) != 0)
    return false;
  ...
  if (blob.size() < 16 + plen)
    return false;
  std::vector<uint8_t> plain;
  if (!unprotect_bytes(blob.data() + 16, plen, plain) || plain.size() != 32)
    return false;
...
bool open_account_session(const std::string& account_id,
                          const Profile& profile,
                          const std::array<uint8_t, 32>* derived_key,
                          std::string* err) {
  if (is_account_locked(account_id)) {
    if (err)
      *err = "этот аккаунт уже открыт в другом окне Nyx";
    return false;
  }
  ...
  if (!write_session_lock(account_id)) {
    ...
    if (err)
      *err = "не удалось заблокировать сессию";
    return false;
  }
  ...
  if (derived_key)
    session_derived_key = *derived_key;
...
bool persist_session_profile(std::string* err) {
  if (!session_open || !session_has_key)
    return false;
  return save_encrypted_profile_with_key(
      profile_path_for(active_id), session_profile, session_derived_key, err);
}

bool write_recovery_vault(const std::string& account_id,
                          const Profile& profile,
...
bool provision_account(const Profile& profile,
  ...
  if (ec) {
    if (err)
      *err = "не удалось создать каталог аккаунта";
    return false;
  }

  std::array<uint8_t, 32> key {};
  if (!save_encrypted_profile(profile_path_for(id), profile, password, err, &key))
    return false;

  const std::string phrase = generate_recovery_phrase();
  std::string normalized;
  if (!normalize_recovery_phrase(phrase, &normalized, err))
    return false;
  if (!write_recovery_vault(id, profile, normalized, err))
    return false;
  ...
  if (!ensure_account_registered(meta)) {
    if (err)
      *err = "не удалось обновить реестр аккаунтов";
    return false;
  }
  if (created)
    *created = meta;
  if (recovery_phrase_out)
    *recovery_phrase_out = normalized;
...
bool unlock_account(const std::string& account_id,
  ...
  if (account_id_from_profile(profile) != account_id) {
    if (err)
      *err = "идентификатор аккаунта не совпадает";
    return false;
  }
  if (!open_account_session(account_id, profile, &key, err))
    return false;
  if (remember_me) {
...
bool try_unlock_remembered(const std::string& account_id, Profile* profile_out, std::string* err) {
  ...
  if (!read_remember_token(account_id, key, nullptr)) {
    if (err)
      *err = "нет активной сохранённой сессии";
    return false;
  }
  ...
  if (!load_encrypted_profile_with_key(profile_path_for(account_id), profile, key, &local_err)) {
    clear_remember_token(account_id);
    if (err)
      *err = local_err.empty() ? "сохранённая сессия устарела" : local_err;
    return false;
  }
  if (account_id_from_profile(profile) != account_id) {
    clear_remember_token(account_id);
...
bool reset_password_with_recovery(const std::string& account_id,
  ...
  std::string normalized;
  if (!normalize_recovery_phrase(recovery_phrase, &normalized, err))
    return false;
  if (!account_has_recovery(account_id)) {
    if (err)
      *err = "для этого аккаунта нет recovery-фразы";
    return false;
  }

  Profile profile;
  if (!load_encrypted_profile(recovery_path_for(account_id), profile, normalized, err)) {
    if (err && err->find("пароль") != std::string::npos)
      *err = "неверная recovery-фраза";
    return false;
  }
  if (account_id_from_profile(profile) != account_id) {
    if (err)
  ...
  if (!write_recovery_vault(account_id, profile, normalized, err))
    return false;
  clear_remember_token(account_id);
  return true;
}
...
std::string active_account_id() {
  return session_open ? active_id : std::string {};
}
...
bool import_legacy_profile(const std::string& password,
  ...
  if (!load_profile(legacy_profile_path(), profile)) {
    if (err)
      *err = "старый profile.json не найден или повреждён";
    return false;
  }
  if (!provision_account(profile, password, recovery_phrase_out, created, err))
    return false;
