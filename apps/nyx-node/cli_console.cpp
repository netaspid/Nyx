...
CliConsole::CliConsole(std::string self_nickname) : self_nickname_(std::move(self_nickname)) {}
...
std::string CliConsole::format_time(uint64_t timestamp_ms) {
  ...
  oss << std::setfill('0') << std::setw(2) << tm_local.tm_hour << ':' << std::setw(2)
      << tm_local.tm_min;
  return oss.str();
}
...
void CliConsole::print_header(const std::string& peer_nickname, const std::string& peer_id_short) {
  std::lock_guard<std::mutex> lock(mutex_);
  std::cout << "\n"
            << "────────────────────────────────────────\n"
            << "  Nyx · " << self_nickname_ << "\n"
...
void CliConsole::print_event(const std::string& text) {
  const auto ts =
      format_time(static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                            std::chrono::system_clock::now().time_since_epoch())
                                            .count()));
  write_line('[' + ts + "] * " + text);
}

void CliConsole::print_message(uint64_t timestamp_ms,
