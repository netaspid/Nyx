...
bool parse_args(int argc, char** argv, nyx::RendezvousServerConfig& cfg) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg.rfind("--bind=", 0) == 0) {
      std::string host;
      uint16_t port = 0;
      if (!nyx::parse_host_port(arg.substr(7), host, port))
        return false;
      cfg.bind_host = host;
      cfg.bind_port = port;
      ...
      cfg.rate_limit_per_minute = static_cast<std::uint32_t>(std::stoul(arg.substr(13)));
    }
  }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  nyx::log_info("rendezvous listening on " + cfg.bind_host + ':' + std::to_string(cfg.bind_port));
