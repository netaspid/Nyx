...
void run_install(HWND hwnd) {
  ...
  if (!ok) {
    MessageBoxW(hwnd,
                (err + L"\n\nЕсли Nyx уже установлен — закройте приложение и повторите.").c_str(),
                L"Nyx Setup",
                MB_ICONERROR);
  ...
  nyx_setup::ensure_document_dependencies(dir, &deps_err, [hwnd](int pct, const wchar_t* status) {
    set_progress(pct);
    if (status)
  ...
  const int launch = MessageBoxW(
      hwnd, L"Nyx установлен.\n\nЗапустить сейчас?", L"Nyx Setup", MB_YESNO | MB_ICONINFORMATION);
  if (launch == IDYES)
    nyx_setup::launch_app(app_exe);
  DestroyWindow(hwnd);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_CREATE: {
    ...
    CreateWindowW(L"STATIC",
                  ...
                  18,
                  hwnd,
                  reinterpret_cast<HMENU>(IDC_PATH_LABEL),
                  nullptr,
                  nullptr);
    g_path_edit = CreateWindowExW(WS_EX_CLIENTEDGE,
                                  L"EDIT",
                                  nyx_setup::default_install_dir().c_str(),
                                  ...
                                  24,
                                  hwnd,
                                  reinterpret_cast<HMENU>(IDC_PATH),
                                  nullptr,
                                  nullptr);
    CreateWindowW(L"BUTTON",
                  ...
                  nullptr,
                  nullptr);
    g_desktop_chk = CreateWindowW(L"BUTTON",
                                  ...
                                  22,
                                  hwnd,
                                  reinterpret_cast<HMENU>(IDC_DESKTOP),
                                  nullptr,
                                  nullptr);
    SendMessageW(g_desktop_chk, BM_SETCHECK, BST_CHECKED, 0);
    g_progress = CreateWindowW(PROGRESS_CLASSW,
                               nullptr,
                               WS_CHILD | PBS_SMOOTH,
                               20,
                               172,
    ...
    g_status = CreateWindowW(L"STATIC",
                             ...
                             nullptr,
                             nullptr);
    g_install_btn = CreateWindowW(L"BUTTON",
                                  L"Установить",
                                  ...
                                  30,
                                  hwnd,
                                  reinterpret_cast<HMENU>(IDC_INSTALL),
                                  nullptr,
                                  nullptr);
    CreateWindowW(L"BUTTON",
                  L"Отмена",
  ...
  case WM_COMMAND:
    ...
    case IDC_BROWSE: {
      ...
      if (nyx_setup::browse_for_folder(hwnd, path))
        SetWindowTextW(g_path_edit, path.c_str());
      return 0;
    }
    case IDC_INSTALL:
      run_install(hwnd);
