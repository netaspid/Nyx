...
bool write_file_bytes(const std::wstring& path,
                      const void* data,
                      std::size_t size,
                      std::wstring* err) {
  ...
  if (Process32FirstW(snap, &pe)) {
    do {
      if (_wcsicmp(pe.szExeFile, exe_name) != 0)
        continue;
      if (!process_exe_matches_dir(pe.th32ProcessID, install_dir))
        continue;
...
bool stop_nyx_for_install(const std::wstring& install_dir, std::wstring* err) {
  ...
  for (const wchar_t* name : names) {
    if (terminate_by_name_in_dir(name, install_dir))
      any = true;
  }
  if (any)
    Sleep(1000);
...
bool extract_payload(const std::vector<std::uint8_t>& blob,
  ...
  if (!parse_payload(blob, files)) {
    if (err)
      *err = L"Invalid installer payload";
    return false;
  }
  ...
  for (size_t i = 0; i < total; ++i) {
    ...
    if (!ensure_parent_dirs(full)) {
      if (err)
        *err = L"Cannot create folder: " + full;
      return false;
    }
    if (f.data.empty()) {
...
bool register_uninstall(const std::wstring& install_dir, const std::wstring& uninstall_exe) {
  ...
  if (RegCreateKeyExW(
          HKEY_CURRENT_USER, subkey.c_str(), 0, nullptr, 0, KEY_WRITE, nullptr, &key, nullptr) !=
      ERROR_SUCCESS) {
    return false;
  }
  ...
  RegSetValueExW(key,
                 ...
                 0,
                 REG_SZ,
                 reinterpret_cast<const BYTE*>(L"Nyx"),
                 static_cast<DWORD>((wcslen(L"Nyx") + 1) * sizeof(wchar_t)));
  RegSetValueExW(key,
                 L"DisplayVersion",
                 0,
                 REG_SZ,
                 reinterpret_cast<const BYTE*>(L"0.1.0"),
...
bool launch_app(const std::wstring& exe_path) {
  return reinterpret_cast<intptr_t>(ShellExecuteW(
             nullptr, L"open", exe_path.c_str(), nullptr, nullptr, SW_SHOWNORMAL)) > 32;
}
...
bool windows_build_at_least(DWORD major, DWORD minor, DWORD build) {
  ...
  if (ver.dwMajorVersion != major)
    return ver.dwMajorVersion > major;
  if (ver.dwMinorVersion != minor)
    return ver.dwMinorVersion > minor;
...
bool ensure_system_prerequisites(std::wstring* err) {
  if (!windows_build_at_least(10, 0, 0)) {
    if (err) {
      *err = L"Nyx требует Windows 10 или новее. Обновите систему через «Параметры → Обновление и "
             L"безопасность».";
    }
    return false;
  }
  ...
  if (info.wProcessorArchitecture != PROCESSOR_ARCHITECTURE_AMD64) {
    if (err)
      *err = L"Nyx работает только на 64-bit Windows.";
    return false;
  }
  return true;
}
...
std::wstring app_paths_soffice_w(HKEY root) {
  ...
  if (RegOpenKeyExW(root,
                    L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\soffice.exe",
                    0,
                    access,
                    &key) != ERROR_SUCCESS) {
    return {};
...
std::wstring find_soffice_exe_w() {
  ...
  if (SearchPathW(nullptr, L"soffice", L".exe", MAX_PATH, buf, nullptr) > 0 && path_exists_w(buf)) {
    return buf;
  }
  ...
  for (const wchar_t* pattern : kCandidates) {
    ...
    if (!path.empty() && path_exists_w(path))
      return path;
  }
  return {};
}
...
bool has_pdf_tools_w(const std::wstring& install_dir) {
  if (tool_on_path_w(L"mutool") || tool_on_path_w(L"pdftoppm"))
    return true;
  if (!install_dir.empty()) {
    if (path_exists_w(install_dir + L"\\tools\\mutool.exe"))
      return true;
    if (path_exists_w(install_dir + L"\\tools\\pdftoppm.exe"))
      return true;
  }
  return false;
}
...
bool has_office_tools_w() {
  return !find_soffice_exe_w().empty();
}
...
bool expand_archive_ps(const std::wstring& zip, const std::wstring& dest_dir) {
  ...
  const std::wstring cmd = L"powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "
...
bool install_mupdf_tools(const std::wstring& install_dir, std::wstring* err) {
  ...
  if (GetTempPathW(MAX_PATH, temp_path) == 0) {
    if (err)
      *err = L"Не удалось получить временный каталог";
    return false;
  }
  ...
  if (!download_file_ps(url, zip)) {
    if (err) {
      *err = L"Не удалось скачать MuPDF. Установите вручную (winget install ArtifexSoftware.MuPDF) "
  ...
  if (!expand_archive_ps(zip, extract)) {
    if (err)
      *err = L"Не удалось распаковать MuPDF";
    DeleteFileW(zip.c_str());
    return false;
  }
...
bool ensure_document_dependencies(const std::wstring& install_dir,
  ...
  if (progress)
    progress(96, L"Зависимости: LibreOffice…");
...
bool verify_installation(const std::wstring& install_dir, std::wstring* err) {
  ...
  if (!path_exists(app)) {
    if (err)
      *err = L"nyx-app.exe not found after install";
    return false;
  }
  ...
  for (const wchar_t* rel : required) {
    ...
    if (!path_exists(full)) {
      if (err)
        *err = L"Missing component: " + std::wstring(rel);
      return false;
    }
  }
