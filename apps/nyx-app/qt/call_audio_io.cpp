...
#define NYX_AUDIO_LOG(...) __android_log_print(ANDROID_LOG_INFO, "NyxAudio", __VA_ARGS__)
#else
...
QVariantList devicesToVariant(const QList<QAudioDevice>& list) {
  ...
  for (const auto& d : list) {
    ...
    if (text.isEmpty())
      text = QString::fromUtf8(d.id());
    if (d.isDefault())
      text += QStringLiteral(" (по умолчанию)");
...
QAudioFormat makeVoipFormat(const QAudioDevice& dev) {
  ...
  for (int rate : rates) {
    ...
    if (dev.isFormatSupported(stereo))
      return stereo;
  }
  QAudioFormat near = dev.preferredFormat();
  if (near.channelCount() <= 0)
    near.setChannelCount(1);
  if (near.sampleRate() <= 0)
    near.setSampleRate(nyx::kCallAudioSampleRate);
  return near;
}
...
QAudioFormat makeCaptureFormat(const QAudioDevice& dev) {
  ...
  if (native.sampleRate() > 0 && native.channelCount() > 0 &&
      native.sampleFormat() != QAudioFormat::Unknown && dev.isFormatSupported(native)) {
    return native;
  }
  return makeVoipFormat(dev);
}
...
std::vector<int16_t> audioBytesToMono16(const QByteArray& bytes, const QAudioFormat& fmt) {
  ...
  auto sample_at = [&](int index) -> int32_t {
    ...
    case QAudioFormat::Float: {
      ...
      if (!std::isfinite(value))
        return 0;
      return static_cast<int32_t>(std::clamp(value, -1.0f, 1.0f) * static_cast<float>(INT16_MAX));
    }
    case QAudioFormat::UInt8:
      return (static_cast<int32_t>(reinterpret_cast<const uint8_t*>(bytes.constData())[index]) -
              128)
             << 8;
    default:
      return 0;
  ...
  if (channels > 1) {
    ...
    selected_channel =
        static_cast<int>(std::max_element(energy.begin(), energy.end()) - energy.begin());
  }

  std::vector<int16_t> mono(static_cast<std::size_t>(frames));
  for (int i = 0; i < frames; ++i) {
    mono[static_cast<std::size_t>(i)] = static_cast<int16_t>(
        std::clamp<int32_t>(sample_at(i * channels + selected_channel), INT16_MIN, INT16_MAX));
  }
  return mono;
}

void resampleMono(
    const int16_t* in, int in_n, int in_rate, std::vector<int16_t>* out, int out_rate) {
  out->clear();
  if (!in || in_n <= 0 || in_rate <= 0 || out_rate <= 0)
    return;
  if (in_rate == out_rate) {
  ...
  for (int i = 0; i < out_n; ++i) {
    ...
    const double s = (1.0 - t) * static_cast<double>(in[i0]) + t * static_cast<double>(in[i1]);
    (*out)[static_cast<std::size_t>(i)] = static_cast<int16_t>(std::clamp(s, -32768.0, 32767.0));
  }
}
...
void CallAudioIo::setPreferredInputId(const QString& id) {
  if (!ensureOnAudioThread("setPreferredInputId")) {
    QMetaObject::invokeMethod(
        this, [this, id]() { setPreferredInputId(id); }, Qt::QueuedConnection);
    return;
  }
...
void CallAudioIo::setPreferredOutputId(const QString& id) {
  if (!ensureOnAudioThread("setPreferredOutputId")) {
    QMetaObject::invokeMethod(
        this, [this, id]() { setPreferredOutputId(id); }, Qt::QueuedConnection);
    return;
  }
...
bool CallAudioIo::openDevices() {
  ...
  pulse_attr.fragsize = static_cast<uint32_t>(nyx::kCallAudioFrameSamples * sizeof(int16_t));
  for (const QAudioDevice& in_dev : inputCandidates(preferred_input_id_)) {
    if (in_dev.isNull())
      continue;
    const QByteArray device_id = in_dev.id();
    int pulse_error = 0;
    pa_simple* capture = pa_simple_new(nullptr,
                                       "Nyx",
                                       PA_STREAM_RECORD,
                                       device_id.isEmpty() ? nullptr : device_id.constData(),
                                       "Voice capture",
                                       &pulse_spec,
                                       nullptr,
                                       &pulse_attr,
  ...
  if (!opened_in) {
    for (const QAudioDevice& in_dev : inputCandidates(preferred_input_id_)) {
      ...
      if (in_fmt.sampleFormat() == QAudioFormat::Unknown)
        continue;
      auto src = std::make_unique<QAudioSource>(in_dev, in_fmt);
      const int bytes_20ms_in = in_fmt.bytesForDuration(nyx::kCallAudioFrameMs * 1000);
  ...
  const int bytes_20ms_out = out_fmt.bytesForDuration(nyx::kCallAudioFrameMs * 1000);
...
void CallAudioIo::pushCapturePcm(const int16_t* samples, int count) {
  ...
  const int need_dev =
      std::max(1,
               (nyx::kCallAudioFrameSamples * capture_rate_ + nyx::kCallAudioSampleRate - 1) /
                   nyx::kCallAudioSampleRate);
  while (static_cast<int>(capture_pcm_.size()) >= need_dev) {
    std::vector<int16_t> frame48;
    resampleMono(capture_pcm_.data(), need_dev, capture_rate_, &frame48, nyx::kCallAudioSampleRate);
    ...
    opus_pcm_.insert(
        opus_pcm_.end(), frame48.begin(), frame48.begin() + nyx::kCallAudioFrameSamples);
  }

  while (static_cast<int>(opus_pcm_.size()) >= nyx::kCallAudioFrameSamples) {
    double energy = 0.0;
    for (int i = 0; i < nyx::kCallAudioFrameSamples; ++i) {
      const double sample = static_cast<double>(opus_pcm_[static_cast<std::size_t>(i)]) / 32768.0;
      energy += sample * sample;
    }
    const float rms = static_cast<float>(std::sqrt(energy / nyx::kCallAudioFrameSamples));
    local_voice_level_.store(static_cast<uint8_t>(std::clamp(rms * 700.0f, 0.0f, 255.0f)),
                             std::memory_order_release);
    const qint64 now = QDateTime::currentMSecsSinceEpoch();
    if (rms >= 0.015f)
      local_voice_last_ms_ = now;
    const bool voice = now - local_voice_last_ms_ <= 300 && !muted_.load(std::memory_order_acquire);
    ...
    opus_pcm_.erase(opus_pcm_.begin(), opus_pcm_.begin() + nyx::kCallAudioFrameSamples);
    if (!packet || !send_fn_)
      continue;
    if (!send_fn_(*packet))
      send_failed_.store(true, std::memory_order_release);
  }
}
...
void CallAudioIo::onCaptureReady() {
  ...
  if (!mic_test_.load(std::memory_order_acquire))
    mixRemoteAudio();
  if (!send_fn_ && !mic_test_.load(std::memory_order_acquire))
    return;
  ...
  if (pulse_capture_) {
    ...
    int pulse_error = 0;
    if (pa_simple_read(static_cast<pa_simple*>(pulse_capture_),
                       pulse_pcm.data(),
                       pulse_pcm.size() * sizeof(int16_t),
                       &pulse_error) < 0) {
  ...
  if (avail < static_cast<qint64>(bytes_per_sample * channels))
    return;
  ...
  if (!mono.empty())
    pushCapturePcm(mono.data(), static_cast<int>(mono.size()));
}
...
void CallAudioIo::stopMicLevelTest() {
  ...
  if (!mic_test_.load(std::memory_order_acquire) && !running_.load(std::memory_order_acquire))
    return;
  stop();
}
...
void CallAudioIo::onRemoteOpus(const QString& peerId, const QByteArray& packet) {
  if (!ensureOnAudioThread("onRemoteOpus")) {
    QMetaObject::invokeMethod(
        this, [this, peerId, packet]() { onRemoteOpus(peerId, packet); }, Qt::QueuedConnection);
    return;
  }
  if (packet.isEmpty())
    return;
  ...
  if (!running_.load(std::memory_order_acquire)) {
    ...
    while (pending_remote_.size() > 80)
      pending_remote_.pop_front();
    return;
  }
  auto& peer = remote_peers_[key];
  if (!peer.decoder)
    peer.decoder = std::make_unique<nyx::OpusDecoderWrap>();
  if (!peer.decoder->ok())
    return;
  ...
  while (peer.jitter.size() > 8)
    peer.jitter.pop_front();
}
...
void CallAudioIo::writePlayback(const std::vector<int16_t>& pcm) {
  ...
  if (!sink_dev_)
    return;
  if (playback_rate_ == nyx::kCallAudioSampleRate && sink_->format().channelCount() == 1) {
  ...
  resampleMono(
      pcm.data(), static_cast<int>(pcm.size()), nyx::kCallAudioSampleRate, &play, playback_rate_);
...
void CallAudioIo::mixRemoteAudio() {
  ...
  for (auto it = remote_peers_.begin(); it != remote_peers_.end();) {
    ...
    if (!peer.primed && peer.jitter.size() >= 2)
      peer.primed = true;
    if (peer.primed && !peer.jitter.empty())
      frames.push_back(&peer.jitter.front());
    ++it;
  }
  ...
  double peak = 1.0;
  for (int32_t sample : accum)
    peak = std::max(peak, std::abs(sample * gain));
  const double limiter = peak > 32000.0 ? 32000.0 / peak : 1.0;
  std::vector<int16_t> mixed(accum.size());
  for (std::size_t i = 0; i < accum.size(); ++i) {
    mixed[i] = static_cast<int16_t>(std::clamp(accum[i] * gain * limiter, -32768.0, 32767.0));
  }
...
bool CallAudioIo::ensureOnAudioThread(const char*) {
  return true;
}
bool CallAudioIo::startMicLevelTest() {
  return false;
